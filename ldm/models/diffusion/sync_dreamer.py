...
class UNetWrapper(nn.Module):
    ...
    def forward(self, x, t, clip_embed, volume_feats, x_concat, is_train=False, **kwargs):
        """
        ...
        pred = self.diffusion_model(x, t, clip_embed, source_dict=volume_feats, **kwargs)
        return pred

    def predict_with_unconditional_scale(self, x, t, clip_embed, volume_feats, x_concat, unconditional_scale, **kwargs):
        x_ = torch.cat([x] * 2, 0)
        t_ = torch.cat([t] * 2, 0)
        clip_embed_ = torch.cat([clip_embed, torch.zeros_like(clip_embed)], 0)
        if 'camera_embedding' in kwargs:
            kwargs['camera_embedding'] = torch.cat([kwargs['camera_embedding']] * 2, 0)
        
        v_ = {}
        for k, v in volume_feats.items():
            v_[k] = torch.cat([v, torch.zeros_like(v)], 0)
            #v_[k] = torch.cat([torch.zeros_like(v), torch.zeros_like(v)], 0)
        ...
        s, s_uc = self.diffusion_model(x_, t_, clip_embed_, source_dict=v_, **kwargs).chunk(2)
...
class SpatialVolumeNet(nn.Module):
    ...
    def construct_spatial_volume(self, x, t_embed, v_embed, target_poses, target_Ks):
        ...
        for ni in range(0, N):
            ...
            # print(pose_source_.shape)
            # print(K_source_.shape)
    ...
    def construct_view_frustum_volume(self, spatial_volume, t_embed, v_embed, poses, Ks, target_indices, **kwargs):
        """
        ...
        # Ks_ = Ks[target_indices] # B*TN,3,3
        batch_idices = torch.arange(0, target_indices.shape[0])[:,None].repeat(1, target_indices.shape[1]).long().to(target_indices.device).flatten()
        target_indices = target_indices.flatten()
        poses_ = poses[batch_idices, target_indices]
        Ks_ = Ks[batch_idices, target_indices]
        ...
        if 'frustum_volume_feats' in kwargs:
            volume_feats_dict = kwargs['frustum_volume_feats'](volume_feats, t_embed_, v_embed_)
        else:
            volume_feats_dict = self.frustum_volume_feats(volume_feats, t_embed_, v_embed_)
        #for k,v in volume_feats_dict.items(): print(k,v.shape)
...
class SyncMultiviewDiffusion(pl.LightningModule):
    def __init__(self, unet_config, scheduler_config,
                 ...
                 view_num=16, normal_predict='none', image_size=256,
        ...
        self._init_schedule()
        # self.register_buffer('poses', torch.zeros(96,16,3,4))
        # self.register_buffer('Ks', torch.zeros(96,16,3,3))
        # self._init_multiview()
        self._init_clip_image_encoder()
        #self._init_clip_projection()

        if self.normal_predict!='none':
            self.viewpoint_dim = 10
            if self.normal_predict!='zero123':
                self.camera_embedding = nn.Sequential(
                    nn.Linear(10, 320*4),
                    nn.SiLU(True),
                    nn.Linear(320*4, 320*4),
                )
            else:
                self.viewpoint_dim = 6
                self._init_clip_projection()
            self.normal_volume = SpatialVolumeNet(self.time_embed_dim, self.viewpoint_dim, self.view_num, input_image_size=self.image_size)
        else:
            self._init_clip_projection()
        
        if self.normal_predict=='joint' or self.normal_predict=='zero123':
            self.volume_merge = SpatialTime3DNet(input_dim=128, time_dim=self.time_embed_dim, dims=(64, 128, 256, 512))
            self.volume_frustum = FrustumTV3DNet(64, self.time_embed_dim, self.viewpoint_dim, dims=(64, 128, 256, 512))
        
        self.spatial_volume = SpatialVolumeNet(self.time_embed_dim, self.viewpoint_dim, self.view_num, input_image_size=self.image_size)
        
        if self.normal_predict=='zero123':
            del self.spatial_volume.frustum_volume_feats
            del self.normal_volume.frustum_volume_feats
        
        self.model = UNetWrapper(unet_config, drop_conditions=drop_conditions, drop_scheme=drop_scheme, use_zero_123=use_zero_123)
    ...
    def _init_multiview(self):
        ...
        self.register_buffer('poses', poses[None].repeat(96,1,1,1))
        self.register_buffer('Ks', K[None].repeat(96,1,1,1))
    ...
    def get_viewpoint_embedding(self, dp):
        ...
        """
        if self.normal_predict!='none' and self.normal_predict!='zero123':
            embedding = torch.stack([dp['de'], dp['da'], dp['dz']], -1) # B,2*N,3
            ...
            embedding = torch.cat([embedding.repeat(1,2,1), embed], -1) # B,2*N,5
            embedding = torch.cat([torch.sin(embedding), torch.cos(embedding)], -1) # B,2*N,10
        elif self.normal_predict=='none':
    ...
    def get_target_view_feats(self, x_input, spatial_volume, clip_embed, t_embed, v_embed, target_index, normal=False, **kwargs):
        """
        ...
        if normal:
            frustum_volume_feats, frustum_volume_depth = self.normal_volume.construct_view_frustum_volume(spatial_volume, t_embed, v_embed, self.poses, self.Ks, target_index, **kwargs)
        else:
            frustum_volume_feats, frustum_volume_depth = self.spatial_volume.construct_view_frustum_volume(spatial_volume, t_embed, v_embed, self.poses, self.Ks, target_index, **kwargs)

        # clip
        TN = target_index.shape[1]
        clip_embed_ = clip_embed.unsqueeze(1).repeat(1,TN,1,1).view(B*TN,clip_embed.shape[1],768)
        if self.normal_predict=='none' or self.normal_predict=='zero123':
        ...
        x_input_ = x_input.unsqueeze(1).repeat(1, TN, 1, 1, 1).view(B * TN, _, H, W)
        x_concat = x_input_
    ...
    def training_step(self, batch):
        ...
        # default_image_size = 256
        # ratio = self.image_size/default_image_size
        # Ks = torch.diag(torch.tensor([ratio,ratio,1],dtype=torch.float32))[None].repeat(B,1,1).to(batch['target_K'].device) @ batch['target_K']
        # Ks = Ks[:,None].repeat(1,self.view_num,1,1).to(torch.float32)
        # poses = batch['target_pose'].to(torch.float32)
        '''
        output_dir = Path(self.image_dir) / 'images' / 'val'
        output_dir.mkdir(exist_ok=True, parents=True)
        self.log_image(batch['target_image'].permute(0, 1, 4, 2, 3), batch, self.global_step, output_dir)
        return
        '''
        ...
        x, clip, input_info = self.prepare(batch)
        ...
        # if self.normal_predict=='mix':
        #    normal_predict = torch.rand(1)[0]
        
        if self.normal_predict=='none':
            volume = self.spatial_volume.construct_spatial_volume(x_noisy[:,:N], t_embed, v_embed[:,:N], self.poses, self.Ks)
        elif self.normal_predict=='joint' or self.normal_predict=='zero123':
            volume = self.volume_merge(torch.cat([self.spatial_volume.construct_spatial_volume(x_noisy[:,:N], t_embed, v_embed[:,:N], self.poses, self.Ks),
                                           self.normal_volume.construct_spatial_volume(x_noisy[:,N:], t_embed, v_embed[:,N:], self.poses, self.Ks)], 1), t_embed)
        else:
            volume = [self.spatial_volume.construct_spatial_volume(x_noisy[:,:N], t_embed, v_embed[:,:N], self.poses, self.Ks), self.normal_volume.construct_spatial_volume(x_noisy[:,N:], t_embed, v_embed[:,N:], self.poses, self.Ks)]
            '''
            if normal_predict<0.5:
                volume = self.spatial_volume.construct_spatial_volume(x_noisy[:,:N], t_embed, v_embed[:,:N], self.poses, self.Ks)
            else:
                volume = self.normal_volume.construct_spatial_volume(x_noisy[:,N:], t_embed, v_embed[:,N:], self.poses, self.Ks)
            '''
        '''
        if self.normal_predict:
            normal_volume = self.normal_volume.construct_spatial_volume(x_noisy[:,N:], t_embed, v_embed[:,N:], self.poses, self.Ks)
            volume = self.merge(torch.cat([spatial_volume, normal_volume], 1), t_embed)
        else:
        ...
        if self.normal_predict=='none':
            ...
            clip_embed, volume_feats, x_concat = self.get_target_view_feats(input_info['x'], volume, clip, t_embed, v_embed[:,:N], target_index, normal=False)
            ...
            clip_embed, volume_feats, x_concat = self.get_target_view_feats(input_info['x'], volume, clip, t_embed, v_embed[:,:N], target_index, normal=False, frustum_volume_feats=self.volume_frustum)
            x_noisy_ = x_noisy[torch.arange(B)[:,None],target_index][:,0] # B,4,H,W
            if self.normal_predict=='joint':
                noise_predict = self.model(x_noisy_, time_steps, clip_embed, volume_feats, x_concat, is_train=True, camera_embedding=self.camera_embedding(v_embed[torch.arange(B)[:,None],target_index][:,0])) # B,4,H,W
            else:
                noise_predict = self.model(x_noisy_, time_steps, clip_embed, volume_feats, x_concat, is_train=True)
            noise_target = noise[torch.arange(B)[:,None],target_index][:,0] # B,4,H,W
            
            clip_embed_, volume_feats_, x_concat_ = self.get_target_view_feats(input_info['x'], volume, clip, t_embed, v_embed[:,N:], target_index, normal=True, frustum_volume_feats=self.volume_frustum)
            n_noisy_ = x_noisy[torch.arange(B)[:,None],target_index+N][:,0] # B,4,H,W
            if self.normal_predict=='joint':
                noise_predict_ = self.model(n_noisy_, time_steps, clip_embed_, volume_feats_, x_concat_, is_train=True, camera_embedding=self.camera_embedding(v_embed[torch.arange(B)[:,None],target_index+N][:,0])) # B,4,H,W
            else:
                noise_predict_ = self.model(n_noisy_, time_steps, clip_embed_, volume_feats_, x_concat_, is_train=True)
            noise_target_ = noise[torch.arange(B)[:,None],target_index+N][:,0] # B,4,H,W
            
            rgb_loss = torch.nn.functional.mse_loss(noise_target, noise_predict, reduction='none').mean()
            normal_loss = torch.nn.functional.mse_loss(noise_target_, noise_predict_, reduction='none').mean()
            loss = rgb_loss + normal_loss
            
            self.log('rgb_loss', rgb_loss, prog_bar=True, logger=True, on_step=True, on_epoch=True, rank_zero_only=True)
            self.log('normal_loss', normal_loss, prog_bar=True, logger=True, on_step=True, on_epoch=True, rank_zero_only=True)
            '''
            clip_embed, volume_feats, x_concat = self.get_target_view_feats(input_info['x'], volume, clip, t_embed, v_embed[:,:N], target_index, normal=False, frustum_volume_feats=self.volume_frustum)
            x_noisy_ = x_noisy[torch.arange(B)[:,None],target_index][:,0] # B,4,H,W
            if self.normal_predict=='joint':
            ...
            clip_embed_, volume_feats_, x_concat = self.get_target_view_feats(input_info['x'], volume, clip, t_embed, v_embed[:,N:], target_index, normal=True, frustum_volume_feats=self.volume_frustum)
            x_noisy_ = x_noisy[torch.arange(B)[:,None],target_index+N][:,0] # B,4,H,W
            if self.normal_predict=='joint':
                noise_predict_ = self.model(x_noisy_, time_steps, clip_embed_, volume_feats_, x_concat, is_train=True, camera_embedding=self.camera_embedding(v_embed[torch.arange(B)[:,None],target_index+N][:,0])) # B,4,H,W
            else:
                noise_predict_ = self.model(x_noisy_, time_steps, clip_embed_, volume_feats_, x_concat, is_train=True)
            noise_target = noise[torch.arange(B)[:,None],target_index+N][:,0] # B,4,H,W
            
            normal_loss = torch.nn.functional.mse_loss(noise_target, noise_predict_, reduction='none').mean()
            ...
            clip_embed, volume_feats, x_concat = self.get_target_view_feats(input_info['x'], volume[0], clip, t_embed, v_embed[:,:N], target_index, normal=False)
            x_noisy_ = x_noisy[torch.arange(B)[:,None],target_index][:,0] # B,4,H,W
            # print(self.camera_embedding(v_embed[torch.arange(B)[:,None],target_index][:,0]).shape)
            noise_predict = self.model(x_noisy_, time_steps, clip_embed, volume_feats, x_concat, is_train=True, camera_embedding=self.camera_embedding(v_embed[torch.arange(B)[:,None],target_index][:,0])) # B,4,H,W
            noise_target = noise[torch.arange(B)[:,None],target_index][:,0] # B,4,H,W
            
            clip_embed_, volume_feats_, x_concat_ = self.get_target_view_feats(input_info['x'], volume[1], clip, t_embed, v_embed[:,N:], target_index, normal=True)
            n_noisy_ = x_noisy[torch.arange(B)[:,None],target_index+N][:,0] # B,4,H,W
            noise_predict_ = self.model(n_noisy_, time_steps, clip_embed_, volume_feats_, x_concat_, is_train=True, camera_embedding=self.camera_embedding(v_embed[torch.arange(B)[:,None],target_index+N][:,0])) # B,4,H,W
            noise_target_ = noise[torch.arange(B)[:,None],target_index+N][:,0] # B,4,H,W
            ...
            '''
            if normal_predict<0.5:
                clip_embed, volume_feats, x_concat = self.get_target_view_feats(input_info['x'], volume, clip, t_embed, v_embed[:,:N], target_index)
                x_noisy_ = x_noisy[torch.arange(B)[:,None],target_index][:,0] # B,4,H,W
                noise_predict = self.model(x_noisy_, time_steps, clip_embed, volume_feats, x_concat, is_train=True, camera_embedding=self.camera_embedding(v_embed[torch.arange(B)[:,None],target_index][:,0])) # B,4,H,W
                noise_target = noise[torch.arange(B)[:,None],target_index][:,0] # B,4,H,W
                loss = torch.nn.functional.mse_loss(noise_target, noise_predict, reduction='none').mean()
            else:
                clip_embed_, volume_feats_, x_concat_ = self.get_target_view_feats(input_info['x'], volume, clip, t_embed, v_embed[:,N:], target_index)
                n_noisy_ = x_noisy[torch.arange(B)[:,None],target_index+N][:,0] # B,4,H,W
                noise_predict_ = self.model(n_noisy_, time_steps, clip_embed_, volume_feats_, x_concat_, is_train=True, camera_embedding=self.camera_embedding(v_embed[torch.arange(B)[:,None],target_index+N][:,0])) # B,4,H,W
                noise_target_ = noise[torch.arange(B)[:,None],target_index+N][:,0] # B,4,H,W
                loss = torch.nn.functional.mse_loss(noise_target_, noise_predict_, reduction='none').mean()
            '''
        '''  
        if self.normal_predict:
            clip_embed_, volume_feats_, x_concat_ = self.get_target_view_feats(input_info['x'], volume, clip, t_embed, v_embed[:,N:], target_index)
            n_noisy_ = x_noisy[torch.arange(B)[:,None],target_index+N][:,0] # B,4,H,W
            noise_predict_ = self.model(n_noisy_, time_steps, clip_embed_, volume_feats_, x_concat_, is_train=True) # B,4,H,W
            noise_target_ = noise[torch.arange(B)[:,None],target_index+N][:,0] # B,4,H,W
            loss_simple_ = torch.nn.functional.mse_loss(noise_target_, noise_predict_, reduction='none')
            loss = loss_simple.mean() + loss_simple_.mean()
        else:
        ...
        print()
        print('cc_projection')
        for name, param in self.cc_projection.named_parameters():
            if param.grad is None:
                print(name)
        
        print()
        print('spatial_volume')
        for name, param in self.spatial_volume.named_parameters():
            if param.grad is None:
                print(name)
        
        print()
        print('normal_volume')
        for name, param in self.normal_volume.named_parameters():
            if param.grad is None:
                print(name)
        
        print()
        print('camera_embedding')
        for name, param in self.camera_embedding.named_parameters():
            if param.grad is None:
                print(name)
        ...
        print()
        print('volume_merge')
        for name, param in self.volume_merge.named_parameters():
            if param.grad is None:
                print(name)
                
        print()
        print('volume_frustum')
        for name, param in self.volume_frustum.named_parameters():
            if param.grad is None:
                print(name)
        ...
        print('model')
        for name, param in self.model.named_parameters():
            if 'middle_conditions' in name and param.grad is None:
                print(name)
            if 'output_conditions' in name and param.grad is None:
                print(name)
    ...
    def log_image(self,  x_sample, batch, step, output_dir):
        ...
        '''
        uids = np.load('/public_bme/data/v-xuchf/test/splits.npy',allow_pickle=True).item()['sync']
        names = []
        for i in uids:
    ...
    def save_image(self,  x_sample, batch, output_dir):
        ...
        names.remove('scales.npy')
        #names = np.array(['P202210250000205_lower'])
    ...
    def configure_optimizers(self):
        ...
        if self.normal_predict=='mix':
            paras.append({"params": self.camera_embedding.parameters(), "lr": lr*10.0},)
            paras.append({"params": self.normal_volume.parameters(), "lr": lr*10.0},)
        elif self.normal_predict=='joint':
            self.spatial_volume.frustum_volume_feats = disable_training_module(self.spatial_volume.frustum_volume_feats)
            self.normal_volume.frustum_volume_feats = disable_training_module(self.normal_volume.frustum_volume_feats)
            self.camera_embedding = disable_training_module(self.camera_embedding)
            
            paras.append({"params": self.normal_volume.parameters(), "lr": lr*10.0},)
            paras.append({"params": self.volume_merge.parameters(), "lr": lr*10.0},)
            paras.append({"params": self.volume_frustum.parameters(), "lr": lr*10.0},)
        elif self.normal_predict=='zero123':
            #self.spatial_volume.frustum_volume_feats = disable_training_module(self.spatial_volume.frustum_volume_feats)
            #self.normal_volume.frustum_volume_feats = disable_training_module(self.normal_volume.frustum_volume_feats)
...
class SyncDDIMSampler:
    ...
    def denoise_apply(self, x_target_noisy, input_info, clip_embed, time_steps, index, unconditional_scale, batch_view_num=1, is_step0=False):
        ...
        if self.model.normal_predict=='none':
            volume = self.model.spatial_volume.construct_spatial_volume(x_target_noisy[:,:N], t_embed, v_embed[:,:N], self.model.poses, self.model.Ks)
        elif self.model.normal_predict=='mix':
            volume = [self.model.spatial_volume.construct_spatial_volume(x_target_noisy[:,:N], t_embed, v_embed[:,:N], self.model.poses, self.model.Ks),
                      self.model.normal_volume.construct_spatial_volume(x_target_noisy[:,N:], t_embed, v_embed[:,N:], self.model.poses, self.model.Ks)]
        else:
            volume = self.model.volume_merge(torch.cat([self.model.spatial_volume.construct_spatial_volume(x_target_noisy[:,:N], t_embed, v_embed[:,:N], self.model.poses, self.model.Ks),
                                                self.model.normal_volume.construct_spatial_volume(x_target_noisy[:,N:], t_embed, v_embed[:,N:], self.model.poses, self.model.Ks)], 1), t_embed) 
        '''
        spatial_volume = self.model.spatial_volume.construct_spatial_volume(x_target_noisy[:,:N], t_embed, v_embed[:,:N], self.model.poses, self.model.Ks)
        if self.model.normal_predict:
            normal_volume = self.model.normal_volume.construct_spatial_volume(x_target_noisy[:,N:], t_embed, v_embed[:,N:], self.model.poses, self.model.Ks)
            volume = self.model.merge(torch.cat([spatial_volume, normal_volume], 1), t_embed)
        else:
            volume = spatial_volume
        '''
        e_t = []
        ...
        if self.model.normal_predict=='none':
            for ni in range(0, N, batch_view_num):
                ...
                clip_embed_, volume_feats_, x_concat_ = self.model.get_target_view_feats(x_input, volume, clip_embed, t_embed, v_embed[:,:N], target_indices_, normal=False)
            ...
            for ni in range(0, N, batch_view_num):
                ...
                time_steps_ = repeat_to_batch(time_steps, B, VN)
                target_indices_ = target_indices[ni:ni+batch_view_num].unsqueeze(0).repeat(B,1)
                clip_embed_, volume_feats_, x_concat_ = self.model.get_target_view_feats(x_input, volume[0], clip_embed, t_embed, v_embed[:,:N], target_indices_, normal=False)
                if unconditional_scale!=1.0:
                    noise = self.model.model.predict_with_unconditional_scale(x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_, unconditional_scale, camera_embedding=self.model.camera_embedding(v_embed[:,ni:ni + batch_view_num]).view(B*VN,-1))
                else:
                    noise = self.model.model(x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_, is_train=False, camera_embedding=self.model.camera_embedding(v_embed[:,ni:ni + batch_view_num]).view(B*VN,-1))
                e_t.append(noise.view(B,VN,4,H,W))
        
            for ni in range(0, N, batch_view_num):
                x_target_noisy_ = x_target_noisy[:, N + ni:N + ni + batch_view_num]
                VN = x_target_noisy_.shape[1]
                x_target_noisy_ = x_target_noisy_.reshape(B*VN,C,H,W)

                time_steps_ = repeat_to_batch(time_steps, B, VN)
                target_indices_ = target_indices[ni:ni+batch_view_num].unsqueeze(0).repeat(B,1)
                clip_embed_, volume_feats_, x_concat_ = self.model.get_target_view_feats(x_input, volume[1], clip_embed, t_embed, v_embed[:,N:], target_indices_, normal=True)
                if unconditional_scale!=1.0:
                    noise = self.model.model.predict_with_unconditional_scale(x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_, unconditional_scale, camera_embedding=self.model.camera_embedding(v_embed[:,N + ni:N + ni + batch_view_num]).view(B*VN,-1))
                else:
                    noise = self.model.model(x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_, is_train=False, camera_embedding=self.model.camera_embedding(v_embed[:,N + ni:N + ni + batch_view_num]).view(B*VN,-1))
                e_t.append(noise.view(B,VN,4,H,W))
        else:
            for ni in range(0, N, batch_view_num):
                x_target_noisy_ = x_target_noisy[:, ni:ni + batch_view_num]
                ...
                time_steps_ = repeat_to_batch(time_steps, B, VN)
                target_indices_ = target_indices[ni:ni+batch_view_num].unsqueeze(0).repeat(B,1)
                clip_embed_, volume_feats_, x_concat_ = self.model.get_target_view_feats(x_input, volume, clip_embed, t_embed, v_embed[:,:N], target_indices_, normal=False, frustum_volume_feats=self.model.volume_frustum)
                if self.model.normal_predict=='joint':
                    if unconditional_scale!=1.0:
                        noise = self.model.model.predict_with_unconditional_scale(x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_, unconditional_scale, camera_embedding=self.model.camera_embedding(v_embed[:,ni:ni + batch_view_num]).view(B*VN,-1))
                    else:
                        noise = self.model.model(x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_, is_train=False, camera_embedding=self.model.camera_embedding(v_embed[:,ni:ni + batch_view_num]).view(B*VN,-1))
            ...
            for ni in range(0, N, batch_view_num):
                ...
                time_steps_ = repeat_to_batch(time_steps, B, VN)
                target_indices_ = target_indices[ni:ni+batch_view_num].unsqueeze(0).repeat(B,1)
                clip_embed_, volume_feats_, x_concat_ = self.model.get_target_view_feats(x_input, volume, clip_embed, t_embed, v_embed[:,N:], target_indices_, normal=True, frustum_volume_feats=self.model.volume_frustum)
                if self.model.normal_predict=='joint':
                    if unconditional_scale!=1.0:
                        noise = self.model.model.predict_with_unconditional_scale(x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_, unconditional_scale, camera_embedding=self.model.camera_embedding(v_embed[:,N + ni:N + ni + batch_view_num]).view(B*VN,-1))
                    else:
