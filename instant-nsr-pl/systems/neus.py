...
class NeuSSystem(BaseSystem):
    ...
    def preprocess_data(self, batch, stage):
        ...
        if stage in ['train']:
            ...
            if self.dataset.config.has_normal:
                normal = self.dataset.all_normals[index, y, x].view(-1, self.dataset.all_normals.shape[-1]).to(self.rank)
                rgb_mask = self.dataset.all_color_masks[index, y, x].view(-1).to(self.rank)
            ...
            if self.dataset.config.has_normal:
                ...
                rgb_mask = self.dataset.all_color_masks[index].view(-1).to(self.rank)
                view_weights = None
